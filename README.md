# restaurant-application

## Idea
A restaurant web application that allows users to browse the menu, explore offers, and make reservations quickly and easily.

## Description
The goal of this project is to create a modern and user-friendly restaurant website.
The application focuses on clear navigation, an attractive visual design, and quick access to important information.

## Mockup
[Link](https://www.figma.com/design/gg2Ucry05XbMXxWdvm1F8f/Untitled?node-id=0-1&t=rdJbB52DzLtcUALE-1)

## How to Run the Project

### Prerequisites
- Node.js (LTS version recommended)
- npm

### Steps
1. Open a terminal in the project folder.
2. Install dependencies:

```bash
npm install
```

3. Compile SCSS to CSS once:

```bash
npm run compile:sass
```

4. Or watch SCSS changes and auto-compile:

```bash
npm run watch:sass
```

5. Open [index.html](index.html) in your browser.

If you are not using watch mode, each time you edit [src/styles.scss](src/styles.scss), run this command again:

```bash
npm run compile:sass
```
